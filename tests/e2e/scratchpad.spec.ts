...
/** #4174: covers the card→page→edit→sync happy path for /trusted/scratchpad.
...
test.describe('/trusted/scratchpad — sync loop', () => {
  ...
  test('Trusted card navigates to the scratchpad and loads existing content', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/trusted\/scratchpad$/);
  ...
  test('typing fires a debounced PUT with the new text', async ({ page }) => {
    ...
    await page.goto('/trusted/scratchpad');
    const editor = page.locator('.scratchpad-editor');
    await expect(editor).toBeVisible();
